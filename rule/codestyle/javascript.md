...
<!-- <link rel="stylesheet" type="text/css" href="http://cdn.bootcss.com/bootstrap/3.3.4/css/bootstrap-theme.css"> -->
...
　　[2.2 目录名](#22%E7%9B%AE%E5%BD%95%E5%90%8D)

　　[2.3 结构](#22-%E7%BB%93%E6%9E%84)

　　　　[2.3.1 缩进](#231-%E7%BC%A9%E8%BF%9B)

　　　　[2.3.2 空格](#232-%E7%A9%BA%E6%A0%BC)

　　　　[2.3.3 换行](#233-%E6%8D%A2%E8%A1%8C)

　　　　[2.3.4 语句](#234-%E8%AF%AD%E5%8F%A5)
...
　　　　[2.4.14 工具](#2414-%E5%B7%A5%E5%85%B7)

[3 变量声明](#3-%E5%8F%98%E9%87%8F%E5%A3%B0%E6%98%8E)

　　[3.1 变量](#31-%E5%8F%98%E9%87%8F)

　　[3.2 函数](#32-%E5%87%BD%E6%95%B0)
        [3.2.1 函数声明](#321-%E5%87%BD%E6%95%B0%E5%A3%B0%E6%98%8E)

　　　　[3.2.2 函数长度](#322-%E5%87%BD%E6%95%B0%E9%95%BF%E5%BA%A6)

　　　　[3.2.3 闭包](#323-%E9%97%AD%E5%8C%85)

　　[3.3 null的使用场景](#33-null%E7%9A%84%E4%BD%BF%E7%94%A8%E5%9C%BA%E6%99%AF)

　　[3.4 不适合null的使用场景](#34-%E4%B8%8D%E9%80%82%E5%90%88null%E7%9A%84%E4%BD%BF%E7%94%A8%E5%9C%BA%E6%99%AF)

　　[3.5 undefined使用场景](#35-undefined%E4%BD%BF%E7%94%A8%E5%9C%BA%E6%99%AF)

　　[3.6 安全](#36-%E5%AE%89%E5%85%A8)

　　[3.7 面向对象](#37-%E9%9D%A2%E5%90%91%E5%AF%B9%E8%B1%A1)

　　[3.8 作用域](#38-%E4%BD%9C%E7%94%A8%E5%9F%9F%E7%9B%B8%E5%85%B3)
...
　　　　[3.10.4 动态执行代码](#3104-%E5%8A%A8%E6%80%81%E6%89%A7%E8%A1%8C%E4%BB%A3%E7%A0%81)
...
#####[强制] 目录名参照上一条规则,有复数结构时，要采用复数命名法，比如说：
...
#####[强制] 对于代码中需要进行==逻辑判断的变量，进行强制类型转换或使用===代替 ,仅当判断`null`或 `undefined`时，允许使用`== null`
...
#####[强制] 私有成员变量和方法命名以下划线开头，如：

```javascript
var _this = {};
```

#####[强制] 涉及Android的，一律大写第一个字母 如：
...
##### [建议] 函数参数大于3个时，应以对象形式作为参数集传递

示例：

```javascript
//bad
function setStyle(height,width,color,padding){
    // do something
}
...
/**
 * 设置样式
 * @param {object} options 参数包含[height,width,color,padding]
 */
...
```
##### [强制] "use strict" 决不允许全局使用， 必须放在函数的第一行， 可以用自执行函数包含大的代码段, 如果 "use strict" 在函数外使用， JSLint 和 JSHint 均会报错
...
```
##### [强制]  不要用null来测试一个没声明的变量。



### 3.5 undefined使用场景

##### [强制] 不要直接使用undefined进行变量判断 
...
##### [强制] 禁止通过在iframe使用script进行跨域回调

##### [建议]尽量在可能但不确定出现异常的地方（大量运算，AJAX请求，数组操作或DOM操作等）用`try-catch(e)`来抛出异常，这样有利于规模较大的项目中排查错误,输出异常抛出错误信息
...
#####[强制] this仅用于类成员函数或对象中
#####[强制] 通用全局函数，特别是通用组件代码应将业务逻辑放入闭包中，并通过“命名空间”将其引入
#####[强制] 若函数中使用到全局变量，则访问全局变量时应使用window来引入
#####[强制] 使用优雅的模版写法，避免多行字符串用\加换行的方式或使用+运算连接字符串这种难维护的编码方式



### 3.9 for

#####[强制] 普通`for`循环, 分号后留有一个空格， 判断条件等内的操作符两边不留空格， 前置条件如果有多个，逗号后留一个空格.
...
```
#####[强制] `for-in` 一定要有 `hasOwnProperty` 的判断， 否则 `JSLint` 或者 `JSHint` 都会有一个 `warn`
